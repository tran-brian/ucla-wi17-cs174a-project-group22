...
var shapes_in_use = [], shaders_in_use = [], textures_in_use = [], active_shader, texture_filenames_to_load = [], gl, g_addrs, displayables, render = true;    // ****** GLOBAL VARIABLES *******
  ...
  { 'construct': function( program )
      { Declare_Any_Class( "Shader_Attribute",
        ...
        this.shader_attributes = [ new Shader_Attribute( gl.getAttribLocation( program, "vPosition"), 3, gl.FLOAT, true,  false, 0, 0 ),  // Pointers to all shader
                                   ...
                                   new Shader_Attribute( gl.getAttribLocation( program, "aLifetime"), 1, gl.FLOAT, false,  false, 0, 0 ), // Attributes for particles
                                   new Shader_Attribute( gl.getAttribLocation( program, "aStartPosition"), 3, gl.FLOAT, false,  false, 0, 0 ),
                                   new Shader_Attribute( gl.getAttribLocation( program, "aEndPosition"), 3, gl.FLOAT, false,  false, 0, 0 ),
                                   new Shader_Attribute( gl.getAttribLocation( program, "aOffset"), 2, gl.FLOAT, false,  false, 0, 0 )];
...
Declare_Any_Class( "Canvas_Manager",                      // This class performs all the setup of doing graphics.   It informs OpenGL of which
  { 'construct': function( canvas_id, background_color )  // functions to call during events - such as a key getting pressed or it being time to redraw.
    ...
    //   }
      ...
      'register_display_object': function(name, object) { this.displayables[name] = object; object.init_keys(this.controls); },
      'unregister_display_object': function(name) { if(this.displayables.hasOwnProperty(name)) delete this.displayables[name]; },
      'render': function(time = 0) {
        this.shared_scratchpad.graphics_state.animation_delta_time = time - ( this.prev_time || 0 );
        if( this.shared_scratchpad.animate ) this.shared_scratchpad.graphics_state.animation_time      += this.shared_scratchpad.graphics_state.animation_delta_time;
        this.prev_time = time;
        ...
        if(render) {
          ...
          var keys = Object.keys(this.displayables);
          for( var i = 0; i < keys.length; i++ )
          {
            if(this.displayables.hasOwnProperty(keys[i])) {
              this.displayables[ keys[i] ].display( time );                                 // Draw each registered displayable.
              this.displayables[ keys[i] ].update_strings( this.shared_scratchpad );
            }
          }
        }
        window.requestAnimFrame( this.render.bind( this ) );      // Now that this frame is drawn, request that it happen again as soon as all other OpenGL events are processed.
...
Declare_Any_Class( "Texture",                                                             // Wrap a pointer to a new texture buffer along with a new HTML image object.
  { construct: function(            filename, bool_mipMap, cubemap, bool_will_copy_to_GPU = true )
      { this.define_data_members( { filename, bool_mipMap, cubemap, bool_will_copy_to_GPU,       id: gl.createTexture() } );
        ...
        if(!cubemap) {
          ...
          this.image.onload   = ( function (texture, bool_mipMap)       // This self-executing anonymous function makes the real onload() function
            { return function( )      // Instrctions for whenever the real image file is ready
              { gl.pixelStorei  ( gl.UNPACK_FLIP_Y_WEBGL, true );
                ...
                gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR ); // sampling up vs sampling down
                if( bool_mipMap && texture.isPowerOf2(texture.image.width) && texture.isPowerOf2(texture.image.height))
          ...
          // var image_counter = 0;
          var base = "Common/textures/cubemap";
